// main.bal - Main API service for Kafka WebSocket Dashboard

import ballerina/http;
import ballerina/log;
import ballerina/time;
import ballerina/websocket;
import kafka_websocket_dashboard.kafka_consumer;
import kafka_websocket_dashboard.websocket_service;
...
service /api on new http:Listener(httpPort) {
    ...
    resource function get info() returns json {
        return {
            "serviceName": "Kafka WebSocket Dashboard API",
            "version": "1.0.0",
...
public function main() returns error? {
    log:printInfo("Starting Kafka WebSocket Dashboard API");
