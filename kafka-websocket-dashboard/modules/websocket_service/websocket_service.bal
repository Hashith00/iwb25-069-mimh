// websocket_service.bal - WebSocket service for real-time event streaming

import ballerina/log;
import ballerina/time;
import ballerina/uuid;
import ballerina/websocket;
import kafka_websocket_dashboard.types;
...
public function broadcastMessage(types:WebSocketMessage message) {
    foreach var [connectionId, caller] in activeConnections.entries() {
        do {
            check caller->writeMessage(types:webSocketMessageToJson(message));
        } on fail error e {
...
public function handleClientConnection(websocket:Caller caller) returns error? {
    ...
    check caller->writeMessage(types:webSocketMessageToJson(welcomeMsg));
}
